---
name: spec-init
description: 启动 spec engineering 工作流 / kick off a new project with docs, requirements, design, implementation plan, TDD plan, README, and AGENTS scaffolding. Use when starting from 0 to 1, clarifying requirements vs design, or setting up spec-first / doc-first / tdd-first workflows.
...
# /spec-init — spec engineering 项目启动器

这个 skill 用于把“我想做个项目”变成一套可执行的起步结构，而不是只生成空文件夹。

目标：

- 初始化一个适合 Spec-Driven Development (SDD) + Test-Driven Development (TDD) 的项目骨架
- 生成 `docs/`、`README.md`、`AGENTS.md`、`src/`、`tests/`、`scripts/`
- 用模板帮助开发者区分“需求文档”和“设计文档”
- 在开始编码前，把项目目标、边界、方案、测试策略和任务清单写清楚

## 何时使用

- 用户说“初始化项目”“从 0 到 1 搭个项目骨架”
- 用户想要 `README.md`、`AGENTS.md`、`docs/` 模板
- 用户不知道需求、设计、实现计划的区别
...
- `docs/00-project-intake.md`: 先澄清背景、用户、目标、非目标
- `docs/01-requirements.md`: 写 what / why / success，不写技术实现
- `docs/02-design.md`: 写 how / architecture / trade-offs，不重复产品目标
- `docs/03-implementation-plan.md`: 写里程碑、先后顺序、依赖关系
- `docs/04-tdd-plan.md`: 写如何用测试证明需求落地
- `docs/05-task-breakdown.md`: 写可执行任务，并回链到需求/设计/测试

如果用户是新手，再额外阅读 `references/example-idea-to-docs.md`，用最小例子帮助对方理解这些文档是怎么衔接的。
...
- 只有在目标路径或覆盖行为无法推断时才使用 `question` 工具
...
如果用户没有明确说类型，就根据目标用户、交付物和使用方式推断，并把推断依据写进 `docs/00-project-intake.md`。
...
- `docs/`、`README.md`、`AGENTS.md`、`src/`、`tests/`、`scripts/` 已创建或补齐
- `docs/00-project-intake.md` 至少写清项目目标、目标用户、非目标、待确认问题
- `docs/01-requirements.md` 中至少有一组 `FR-*` 和 `AC-*`
- `docs/02-design.md` 中至少有一组 `DES-*`，并映射到 `FR-*`
- `docs/04-tdd-plan.md` 中至少有一组 `TEST-*`，并映射到 `FR-*`
- `docs/05-task-breakdown.md` 中至少有一组 `T-*`，并映射到 `FR-*` / `DES-*` / `TEST-*`
...
4. 检查仓库现状：
   - 新目录/空目录 → 走完整初始化
   - 已有项目 → 仅补齐缺失文档与基础目录
5. 如果存在同名文件且会覆盖，只有这时再询问用户是否覆盖；否则默认保留原文件

### Step 1: 先做需求澄清，不先做方案

先收集或推断以下信息。如果信息不完整，不要阻塞，写进 `docs/00-project-intake.md` 的 `[待确认]` 区域：
...
```bash
bash "<skill-root>/scripts/spec-init.sh" [目标路径] --name "[项目名]" --type "[项目类型]"
```
...
- `references/doc-boundaries.md`: 解释需求、设计、实现计划、测试计划、任务的边界
